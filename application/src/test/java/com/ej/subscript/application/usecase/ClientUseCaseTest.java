...
class ClientUseCaseTest {
    ...
    void shouldActivateClient() {
        ...
        when(clientRepository.findById(inactive.id())).thenReturn(Mono.just(inactive));
        when(clientRepository.update(any())).thenReturn(Mono.just(ACTIVE_CLIENT));

        StepVerifier.create(clientUseCase.activate(inactive.id()))
                .assertNext(result -> assertThat(result.status()).isEqualTo(ClientStatus.ACTIVE))
                .verifyComplete();
    }
    ...
    void shouldReturn404WhenActivatingNonExistentClient() {
        ...
        StepVerifier.create(clientUseCase.activate(id))
                .expectErrorSatisfies(ex -> {
                    assertThat(ex).isInstanceOf(BusinessException.class);
                    assertThat(((BusinessException) ex).status()).isEqualTo(404);
                })
