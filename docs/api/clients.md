...
| `cedula`   | string  | required, non-blank, unique per owner | tax/national id of the client; the same cedula can exist across different owners but not twice within the same owner — enforced at the use case and at the DB level (compound `UNIQUE (owner_id, cedula)` constraint) |
